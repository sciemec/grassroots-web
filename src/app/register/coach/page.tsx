...
function CoachRegisterForm() {
  ...
  const submit = async () => {
    ...
    const payload = {
      role: "coach", first_name: form.first_name.trim(), surname: form.surname.trim(),
      phone: form.phone.trim(), province: form.province, team_name: form.team_name.trim(),
    ...
    <div className="flex min-h-screen items-start justify-center bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-900 px-4 py-10">
      <div className="w-full max-w-lg">
        ...
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-blue-500 text-2xl shadow-lg">{sportCfg.emoji}</div>
          <div>
        ...
        <div className="mb-6 h-1 w-full rounded-full bg-white/10">
          <div className="h-full rounded-full bg-blue-500 transition-all duration-300" style={{ width: `${((step - 1) / 2) * 100}%` }} />
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-7 backdrop-blur-sm">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-blue-400/70">We&apos;ll send a verification code to this number</p>
              </div>
              <div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div><h2 className="text-xl font-bold text-white">Professional details</h2><p className="text-sm text-blue-300">Your coaching background</p></div>
              <div>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-blue-200">Coaching level</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-blue-200">Years of experience</label>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div><h2 className="text-xl font-bold text-white">Almost done!</h2><p className="text-sm text-blue-300">We&apos;ll verify your number to create your account</p></div>
              ...
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <span className="text-sm text-blue-200">I agree to the <span className="text-blue-400 underline">Terms of Service</span> and <span className="text-blue-400 underline">Privacy Policy</span></span>
              </label>
              <div className="rounded-xl border border-blue-500/20 bg-blue-500/10 px-4 py-3 text-center">
                <p className="text-xs text-blue-300">A 6-digit code will be sent to <span className="font-bold text-white">{form.phone}</span></p>
              </div>
            </div>
          )}

          {status && <div className="mt-4 flex items-center justify-center gap-2 text-sm text-blue-300"><Loader2 className="h-4 w-4 animate-spin" /> {status}</div>}
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button onClick={handleNext} disabled={loading}
              ...
              {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Sending code…</> :
               step === 3 ? <><CheckCircle2 className="h-4 w-4" /> Send Verification Code</> :
               <>Next <ChevronRight className="h-4 w-4" /></>}
            </button>
          </div>
        </div>

        <p className="mt-5 text-center text-sm text-blue-400">Already have an account?{" "}<Link href="/login" className="font-semibold text-white hover:underline">Sign in</Link></p>
      </div>
    </div>
  );
}
...
export default function CoachRegisterPage() {
  return <Suspense><CoachRegisterForm /></Suspense>;
}
