      ...
      <main className="gs-watermark flex-1 overflow-auto p-6">
        ...
        <div className="mb-6 flex items-center justify-between border-b border-white/5 pb-4">
          <div className="flex items-center gap-3">
            <Link href="/coach/dashboard" className="rounded-lg p-1.5 bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-colors">
              <ArrowLeft className="h-4 w-4" />
            </Link>
            <div>
              <h1 className="text-xl font-black text-white uppercase tracking-tight">Set Piece Analytics</h1>
              <p className="text-xs text-white/50 font-semibold">Track corner variables, direct free kicks, and strategic roles</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2">Select Your Active Tactical Specialty Focus</p>
          ...
          <div className={`mt-4 border p-4 rounded-xl flex items-start gap-4 transition-all ${currentGuide.color}`}>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-white/70 leading-relaxed font-medium">{currentGuide.focusText}</p>
              <div className="pt-1.5 space-y-1">
                {currentGuide.routines.map((routine, idx) => (
                  <div key={idx} className="flex gap-2 text-xs text-white/90 font-semibold">
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-xs font-black uppercase tracking-widest transition-all ${
                activeTab === tab
                  ...
                  : "border border-white/10 bg-card/60 text-white/60 hover:bg-white/10"
              }`}
            >
              {tab === "corners" && "Corners"}
          ...
          <div className="space-y-5">
            ...
            {corners.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-card/60 p-4">
                <p className="mb-2 text-xs font-black uppercase text-white/40 tracking-wider">Your Corners vs Opposition</p>
                <div className="flex items-center gap-3 text-xs font-bold text-white">
                  <span className="w-20 text-right">{cornersFor.length} For</span>
                  <div className="flex-1 rounded-full bg-white/5 border h-3 overflow-hidden">
                    <div
            ...
            {showCornerForm && (
              <div className="rounded-xl border border-white/10 bg-card/60 p-5">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <label className="mb-1.5 block text-xs font-bold text-white/60 uppercase">For / Against</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-bold text-white/60 uppercase">Outcome</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-bold text-white/60 uppercase">Delivery Curve</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-bold text-white/60 uppercase">Target Landing Zone</label>
            ...
            {corners.length > 0 && (
              <div className="space-y-2">
                {corners.map((c, i) => (
                  <div key={c.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-card/40 px-4 py-3 text-xs text-white">
                    ...
                    <span className="text-white/50 font-medium ml-auto">{c.delivery} // {c.zone}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {corners.length === 0 && !showCornerForm && (
              <div className="rounded-xl border border-dashed border-white/10 p-10 text-center text-xs text-white/40 font-bold">
        ...
        {activeTab === "free-kicks" && (
          <div className="space-y-5">
            ...
            {showFKForm && (
              <div className="rounded-xl border border-white/10 bg-card/60 p-5">
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-bold text-white/60 uppercase">Distance Boundary</label>
            ...
            {freeKicks.length > 0 && (
              <div className="space-y-2">
                {freeKicks.map((fk, i) => (
                  <div key={fk.id} className="flex items-center gap-3 rounded-xl border border-white/10 bg-card/40 px-4 py-3 text-xs text-white">
                    <span className="text-white/40 font-bold w-6">#{i + 1}</span>
                    ...
                    <span className="text-white/50 font-medium ml-auto">{fk.distance}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === "ai" && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-white/10 bg-card/60 backdrop-blur-md p-5 sm:p-6">
              ...
              {aiLoading && (
                <div className="mt-4 flex items-center gap-3 rounded-xl bg-white/5 border border-white/5 px-4 py-6">
                  <Loader2 className="h-4 w-4 animate-spin text-[#f0b429]" />
                  <p className="text-xs text-white/60 font-bold uppercase tracking-wider animate-pulse">Running advanced tactical computations...</p>
                </div>
              )}
              ...
              {aiReport && !aiLoading && (
                <div className="mt-4 rounded-xl border border-[#1a5c2a]/30 bg-black/30 p-5">
                  ...
                  <div className="space-y-2 text-xs sm:text-sm text-white/90 leading-relaxed whitespace-pre-line font-medium">
                    {aiReport}
                  </div>
                </div>
              )}
              ...
              {!aiReport && !aiLoading && !aiError && (
                <div className="mt-4 rounded-xl border border-dashed border-white/10 px-5 py-8 text-center">
                  <Brain className="mx-auto mb-2 h-7 w-7 text-white/20" />
