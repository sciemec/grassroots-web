...
export default function ScoutReportsPage() {
  ...
  const generateReports = async () => {
    ...
    setError("");
    try {
      const res = await api.post("/scout/reports/generate", {
        player_ids: Array.from(selected),
      });
      const generated: GeneratedReport[] = res.data?.reports ?? [];
      setReports(generated);
      // Auto-download if single PDF returned
      if (res.data?.download_url) {
        window.open(res.data.download_url, "_blank");
      }
    } catch {
      setError("Failed to generate reports. The AI service may be busy — please try again.");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const downloadReport = (report: GeneratedReport) => {
    if (report.download_url) {
      ...
      a.href = url;
      a.download = `scout-report-${report.player_id}.txt`;
      a.click();
    ...
    <div className="flex h-screen bg-background">
      ...
      <main className="flex-1 overflow-auto p-6">
        ...
        <div className="mb-6 flex items-center gap-3">
          <Link href="/scout/shortlist" className="rounded-lg p-1.5 hover:bg-muted transition-colors">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Select players and generate AI-powered scouting reports via Claude</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex items-start gap-3 rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
              ...
              <p className="text-xs text-blue-700">
                Reports are generated by Claude AI using each player&apos;s training history, scores, and positional data.
                Player identities are protected — reports use initials and region only, per ZIFA safeguarding policy.
              </p>
            </div>
            ...
            <div className="rounded-xl border bg-card">
              ...
              {isLoading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">No players found. Build your shortlist first.</p>
                  <Link href="/scout" className="mt-3 inline-block text-sm text-primary hover:underline">Find players →</Link>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm">{player.initials}</p>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border bg-card p-5 sticky top-0">
              ...
              <div className="mb-5 rounded-xl bg-muted/40 p-4 text-center">
                <p className="text-3xl font-black text-primary">{selected.size}</p>
                <p className="text-xs text-muted-foreground">player{selected.size !== 1 ? "s" : ""} selected</p>
              </div>
              ...
              >
                {generating ? (
                  ...
                  <><FileText className="h-4 w-4" /> Generate {selected.size > 0 ? `${selected.size} ` : ""}Report{selected.size !== 1 ? "s" : ""}</>
                )}
              </button>

              <p className="mt-3 text-xs text-center text-muted-foreground">
                Reports include training performance, positional analysis, and development recommendations.
              </p>

              {/* Report cost indicator */}
              <div className="mt-4 rounded-xl border border-dashed p-3 text-center">
                ...
                <p className="text-xs font-medium mt-0.5">Requires active Scout subscription</p>
              </div>
            </div>
            ...
            {reports.length > 0 && (
              <div className="rounded-xl border bg-card p-5">
                <h2 className="font-semibold mb-3 flex items-center gap-2">
                  <CheckCircle className="h-4 w-4 text-green-600" /> Reports Ready
                </h2>
                <div className="space-y-2">
                  {reports.map((report) => {
                    const player = allCandidates.find((p) => p.id === report.player_id);
                    return (
                      <div key={report.player_id} className="flex items-center justify-between rounded-lg border px-4 py-3">
                        <div>
                          <p className="text-sm font-medium">{player?.initials ?? report.player_id}</p>
                          <p className="text-xs text-muted-foreground">{new Date(report.generated_at).toLocaleTimeString("en-ZW")}</p>
                        </div>
                        <button
