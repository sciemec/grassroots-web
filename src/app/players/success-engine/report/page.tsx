    ...
    <div className="min-h-screen bg-[#121212] text-white pb-10">
      ...
      <div className="px-5 pt-10 pb-5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#F9A825]">Weekly Report</p>
          <p className="text-sm text-white/50 truncate">{goal.goalText}</p>
        </div>
        <button
          onClick={handleShare}
          className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"
      ...
      <div className="px-5 space-y-4">
        {/* Completion rate + week grid */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-white/40">This Week</p>
            <span className="text-3xl font-bold text-[#F9A825]">{rate}%</span>
          </div>
          <div className="h-2 rounded-full bg-white/10 mb-4">
            <div className="h-2 rounded-full bg-[#F9A825] transition-all duration-500" style={{ width: `${rate}%` }} />
          </div>
              ...
              <div key={i} className="flex flex-col items-center gap-1">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    day.status === "done" ? "bg-[#1B5E20] text-white"
                    : day.status === "future" ? "bg-white/5 text-white/20 border border-white/10"
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-center">
            <p className="text-xl font-bold" style={{ color: pColor }}>{probability}%</p>
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-[#1B5E20]/40 bg-[#1B5E20]/10 p-4">
                <p className="text-[10px] font-semibold text-[#1B5E20] uppercase tracking-wide mb-1">Strongest ✅</p>
                <p className="text-xs text-white/80 leading-relaxed line-clamp-3">{reportData.strongestAction}</p>
                <p className="text-xs text-[#1B5E20] mt-1 font-semibold">{reportData.strongestDays}/7 days</p>
              </div>
              <div className="rounded-2xl border border-[#B71C1C]/30 bg-[#B71C1C]/10 p-4">
                <p className="text-[10px] font-semibold text-[#B71C1C] uppercase tracking-wide mb-1">Needs Work ⚠️</p>
                <p className="text-xs text-white/80 leading-relaxed line-clamp-3">{reportData.weakestAction}</p>
                <p className="text-xs text-[#B71C1C] mt-1 font-semibold">{reportData.weakestDays}/7 days</p>
              </div>
            </div>

            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs font-semibold uppercase tracking-wide text-white/40 mb-2">Actions This Week</p>
              <p className="text-sm text-white/70">{reportData.totalDone} of {reportData.totalPossible} actions completed</p>
            </div>
          </>
        )}
        ...
        <div className="rounded-2xl border border-[#F9A825]/20 bg-[#F9A825]/5 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-semibold text-[#F9A825] uppercase tracking-wide">THUTO AI Analysis</p>
          </div>
          {!analysis && !loading && (
            <button
          ...
          {loading && (
            <div className="flex items-center gap-2 text-white/40 text-sm">
              <Loader2 size={14} className="animate-spin" /> THUTO is reading your week…
            </div>
          )}
          {error && <p className="text-sm text-[#B71C1C]">{error}</p>}
          {analysis && (
            <div>
              <p className="text-sm text-white/90 leading-relaxed whitespace-pre-line">{analysis}</p>
              <button onClick={generateAnalysis} className="mt-3 text-xs text-[#F9A825]/60 hover:text-[#F9A825] transition-colors">
                Regenerate
              </button>
            </div>
          )}
