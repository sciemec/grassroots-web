...
import { ArrowLeft, UserPlus, UserCheck, MessageCircle, MapPin, Search, Heart, MessageSquare } from "lucide-react";
...
export default function ScoutPublicProfile({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  if (notFound) return (
    <div style={{ minHeight: "100vh", backgroundColor: BG }} className="flex flex-col items-center justify-center gap-4">
      <p className="text-gray-400">Scout profile not found</p>
    ...
    <div style={{ minHeight: "100vh", backgroundColor: BG }}>
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <span className="font-bold text-base" style={{ color: GRS_GREEN }}>Scout Profile</span>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          {/* Gold banner */}
          <div className="h-20" style={{ background: `linear-gradient(135deg, #7c5a00 0%, ${GOLD} 100%)` }} />
          ...
          <div className="px-6 pb-6">
            ...
            <div className="flex flex-wrap gap-1.5 mt-1.5">
              <span className="px-2 py-0.5 rounded-full text-xs font-semibold text-white" style={{ background: GRS_GREEN }}>Scout</span>
              {profile?.sport && (
        ...
        <div className="text-center py-4">
          ...
          <p className="mt-2 text-xs text-gray-400">Free for all Zimbabwean scouts</p>
        </div>
      </div>
    </div>
  );
