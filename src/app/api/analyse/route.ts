...
// POST /api/analyse
// ─────────────────────────────────────────────────────────────────────────────
// Web drill video analysis — Gemini 2.0 Flash (Pro feature, 1 free trial).
//
// Receives multipart form:
//   video   — File (mp4/mov, max 60s)
//   sport   — string  e.g. "football"
//   drill   — string  e.g. "sprint"
//   position — string e.g. "striker"
//   token   — string  JWT from auth store (for credit tracking)
//
// Flow:
//   1. Check credits via Laravel GET /video-analysis/credits
//   2. Upload video bytes to Gemini Files API (resumable)
//   3. Wait for Gemini processing
//   4. Generate coaching feedback
//   5. Record usage via Laravel POST /video-analysis/record
...
// Maps sport + drill → Gemini coaching prompt
function buildContext(sport: string, drill: string, position: string): string {
  ...
  // Try exact key first, then sport-only, then generic
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    sport     = (form.get('sport')    as string) || 'football';
    drill     = (form.get('drill')    as string) || 'drill';
    position  = (form.get('position') as string) || 'player';
  ...
  if (token) {
    try {
      ...
      // If credit check fails, allow — don't block the user
    }
  }
  ...
  const initRes = await fetch(
    `https://generativelanguage.googleapis.com/upload/v1beta/files?key=${GEMINI_API_KEY}`,
    {
      ...
      body: JSON.stringify({ file: { display_name: 'grs_drill.mp4' } }),
      signal: AbortSignal.timeout(20_000),
    }
  );

  if (!initRes.ok) {
    return NextResponse.json({ error: 'Could not start video upload. Try again.' }, { status: 502 });
  }

  const uploadUrl = initRes.headers.get('X-Goog-Upload-URL');
  if (!uploadUrl) {
    return NextResponse.json({ error: 'Upload URL not returned by Gemini.' }, { status: 502 });
  }
  ...
  if (!uploadRes.ok) {
    return NextResponse.json({ error: 'Video upload to AI failed. Try a shorter clip.' }, { status: 502 });
  }
  ...
  if (token) {
    try {
      ...
      // Non-fatal — feedback already generated
    }
  }
