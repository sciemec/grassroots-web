    ...
    <div className="flex h-screen bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto text-white">
        ...
        <div className="mx-auto max-w-2xl space-y-6 px-6 py-6">
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {isPro ? (
                ...
                <Link href="/player/subscription" className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors" style={{ background: "#c8962a", color: "#fff", textDecoration: "none" }}>
                  🔒 Unlock
                </Link>
              )}
            </div>
          ...
          {/* ── Save + Action bar ── */}
          {!isPro && (
            <div className="mb-3 rounded-xl px-4 py-3 text-sm font-semibold" style={{ background: "#fffbeb", border: "1px solid #f0b429", color: "#92400e" }}>
              🔒 Premium Feature — subscribe to save your Talent Passport and export a PDF for scouts
              {" "}<Link href="/player/subscription" className="font-bold underline" style={{ color: "#1a5c2a" }}>View plans →</Link>
            </div>
          )}
          <div className="flex gap-3 pb-8">
            <button
              onClick={isPro ? saveAll : undefined}
              disabled={!isPro}
              className="flex-1 rounded-xl border py-3 text-sm font-bold transition-colors"
              style={{
                borderColor: isPro ? "rgba(52,211,153,0.4)" : "#e5e7eb",
                background: isPro ? "rgba(6,78,59,0.3)" : "#f3f4f6",
                color: isPro ? "rgb(110,231,183)" : "#9ca3af",
                cursor: isPro ? "pointer" : "not-allowed",
              }}
            >
              {isPro ? (saved ? "✓ Saved!" : "Save Passport") : "🔒 Save Passport"}
            </button>
            <button
              onClick={isPro ? exportPDF : undefined}
              disabled={!isPro || exporting}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold disabled:opacity-50 transition-colors"
              style={{
                background: isPro ? "#f0b429" : "#d1d5db",
                color: isPro ? "#1a3a1a" : "#6b7280",
                cursor: isPro ? "pointer" : "not-allowed",
              }}
            >
              {exporting
              ...
              {exporting ? "Exporting…" : isPro ? "Export PDF" : "🔒 Export PDF"}
            </button>
          </div>
        </div>
      </main>
