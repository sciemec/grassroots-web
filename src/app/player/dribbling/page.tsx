...
interface HistoryEntry {
  ...
  body_position_score:       number;
  change_of_direction_score: number;
  ...
  const fetchCoachRating = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const match = list.find((x: { skill_code: string }) => x.skill_code === "dribbling");
...
Return this exact JSON structure:
{
  ...
  "skill_tip": "<one tip specific to ${dribbleType} dribbling>"
    ...
    postToArena(`Analysed ${dribbleType} dribbling — scored ${computed}/100`, {
      postType: "milestone",
      activityType: "skill_analysis",
      activityData: { skill: "dribbling", type: dribbleType, score: computed, sport },
    });
    setPhase("results");
    setLoading(false);
  };
                ...
                <div key={entry.id} style={{ backgroundColor: "white", borderRadius: 12, border: "1px solid #e5e7eb", marginBottom: 10, overflow: "hidden" }}>
                  <button onClick={() => setOpenHist(isOpen ? null : entry.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: "0 0 2px", fontSize: 14, fontWeight: 600, color: "#111" }}>{entry.sport} · {entry.dribble_type}</p>
                  ...
                  {isOpen && (
                    <div style={{ padding: "0 16px 16px", borderTop: "1px solid #f3f4f6" }}>
                      <div style={{ paddingTop: 14 }}>
                        {[
                          { label: "Ball Control & Touch", score: entry.ball_control_score, color: "#1a5c2a" },
                          { label: "Body Position", score: entry.body_position_score, color: "#2563eb" },
                          { label: "Change of Direction", score: entry.change_of_direction_score, color: "#c8962a" },
                          { label: "Head Up & Awareness", score: entry.awareness_score, color: "#7c3aed" },
                        ].map((m) => {
                      ...
                      {entry.ai_feedback?.skill_tip && (
                        <p style={{ margin: "10px 0 0", fontSize: 12, color: "#6b7280" }}><strong>Skill tip:</strong> {entry.ai_feedback.skill_tip}</p>
                      )}
                    </div>
                  )}
                </div>
