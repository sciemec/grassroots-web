...
import { ArrowLeft, Send, ChevronDown, ChevronUp, RotateCcw, History, Video } from "lucide-react";
...
export default function PassingAnalyzerPage() {
  ...
  const analyseVideoFile = async () => {
    ...
    try {
      const r = await fetch("/api/fitness-test?test_type=passing&age_group=u17", {
        method: "POST",
        body: form,
      });
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "28px 16px" }}>
          ...
          {inputMode === "video" && (
            <div style={{ border: "2px dashed #16a34a", borderRadius: 12, padding: "24px 16px", marginBottom: 20, textAlign: "center", backgroundColor: "#f0fdf4" }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: 13, color: "#374151", fontWeight: 500 }}>Upload a short passing clip (max 60s)</p>
              ...
              <p style={{ margin: "10px 0 0", fontSize: 12, color: "#6b7280" }}>AI measures body shape &amp; follow-through. Rate the rest manually.</p>
            </div>
          )}
