...
// ─────────────────────────────────────────────────────────────────────────────
// DRILL CONTENT — 5 positions, sourced from England FA / Catalan / Costa Rica
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// TIER CONFIG — from GRS unlock system (drills-route.ts)
// ─────────────────────────────────────────────────────────────────────────────
const TIER_CONFIG: Record<number, {
  label: string; color: string; bg: string; source: string; flag: string;
}> = {
...
// ─────────────────────────────────────────────────────────────────────────────
// WEEKLY CHALLENGES — from new DrillsPage

// ─────────────────────────────────────────────────────────────────────────────
// TIER PROGRESS — shape from GET /api/drills?playerId=xxx
// ─────────────────────────────────────────────────────────────────────────────
...
export default function FootballDrillsLabPage() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      try {
        // Load existing drill completion progress
        const progressRes = await fetch("/api/player/progress");
        if (progressRes.ok) {
          const data = await progressRes.json();
          setCompletedDrills(data.drills ?? []);
          if (data.position) {
            setActivePosition(data.position as keyof typeof FOOTBALL_POSITION_DRILLS);
          } else {
            mapUserPosition();
          }
        } else {
        ...
        // Load tier progress from GRS unlock system
  ...
  const saveProgress = async (newCompleted: string[], position: string) => {
    ...
    try {
      ...
      // Also mark as complete in the GRS unlock system
      const playerId = user?.id ?? "me";
  ...
  // ── Talent passport JSON download — from existing page ───────────────────
          ...
          <div className="max-w-5xl mx-auto mt-3 pb-1">
            ...
            <div className="h-1.5 bg-white/15 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500"
                style={{
                  background: tierProgress.nextTier.unlockReady ? "#f0b429" : "#4ade80",
                  width: `${tierProgress.nextTier.unlockReady ? 100 : Math.max(5, 100 - (tierProgress.nextTier.sessionsNeeded / 8) * 100)}%`,
                }} />
            </div>
          </div>
        )}
      ...
      <main className="max-w-5xl mx-auto p-4 sm:p-6 space-y-5">
        ...
        {activeTab === "drills" && (
          <>
            ...
            {/* Main drill layout — info panel + list */}
              ...
              <div className="lg:col-span-4 bg-white border border-gray-200 rounded-3xl p-5 shadow-sm space-y-4 lg:sticky lg:top-44">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded border"
                      style={{ background: tierCfg.bg, color: tierCfg.color, borderColor: tierCfg.bg }}>
                ...
                {completionPct === 100 && (
                  <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-xl p-3 text-center">
                    ...
                    <p className="text-[9px] text-yellow-700">All drills completed for this position</p>
                  </div>
                )}
                ...
                {/* Next tier teaser inside info panel */}
                  ...
                  <div className="border border-dashed border-gray-200 rounded-xl p-3 text-center">
                    ...
                    {tierProgress.nextTier.sessionsNeeded > 0 && (
                      <div className="text-[10px] text-gray-500 mt-1">
                        {tierProgress.nextTier.sessionsNeeded} more session{tierProgress.nextTier.sessionsNeeded > 1 ? "s" : ""}
                      </div>
                    )}
                  </div>
                )}
                ...
                {tierProgress?.nextTier?.unlockReady && (
                  <div className="bg-[#f0b429] rounded-xl p-3 text-center">
                    <div className="text-xs font-black text-[#1c3d22] mb-1">
                      🎉 {tierProgress.nextTier.label} tier ready!
                    </div>
                    <div className="text-[10px] text-[#1c3d22] mb-2">{tierProgress.nextTier.source} unlocked</div>
                    <Link href="/player/talent-id"
                      className="text-[10px] font-black text-[#1c3d22] underline">
                      Take a test to activate →
                    </Link>
                  </div>
                )}
              </div>
                        ...
                        <div className="border-t border-gray-100 px-5 py-4 bg-gray-50/50 space-y-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button onClick={() => toggleDrillCompletion(drill.id)}
                              className={`text-xs font-black uppercase tracking-wider px-4 py-2.5 rounded-xl border transition-all cursor-pointer flex items-center gap-1.5 ${
                                isDone
                                  ? "bg-emerald-600 text-white border-emerald-600"
                                  : "bg-[#1c3d22] text-white border-[#1c3d22]"
                              }`}>
                              {isDone ? <><CheckCircle2 size={12} /> Completed</> : <><Play size={12} className="fill-current" /> Mark as done</>}
                            </button>
                            <Link href="/player/talent-id"
                              className="text-xs font-semibold px-4 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:border-gray-300 transition-colors">
                              Test after drilling →
                            </Link>
                          </div>
                        </div>
            ...
            {/* Talent passport summary — shown when drills are started */}
            {completedCount > 0 && (
        ...
        {activeTab === "challenges" && (
          <WeeklyChallenges
            playerSessionCount={tierProgress?.totalCompleted ?? 0}
          />
        )}

        {/* ── TAB: MY COACH (THUTO / AMARA) ─────────────────────────── */}
        {activeTab === "coach" && (
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">
                    Your GRS AI coach · {tierProgress?.currentTierLabel ?? "Spark"} tier · {activePosition}
                  </div>
                </div>
              </div>

              {tipLoading && (
                <p className="text-sm text-gray-400 italic">Getting your coaching tip...</p>
              )}
              {coachTip && !tipLoading && (
                <div className="text-sm text-gray-700 leading-relaxed p-3 bg-gray-50 rounded-xl border-l-4 border-[#1c3d22]">
                  {coachTip}
                </div>
              )}
              {!coachTip && !tipLoading && (
                <button onClick={fetchCoachTip}
                  className="w-full py-3 rounded-xl bg-[#1c3d22] text-white font-black text-sm border-none cursor-pointer hover:bg-[#2a5532] transition-colors">
...
// ─────────────────────────────────────────────────────────────────────────────
// INLINE COACH CHAT — reusable chat component for the coach tab
// ─────────────────────────────────────────────────────────────────────────────
function CoachChat({ coachName, tier, position, gender }: {
  coachName: string; tier: string; position: string; gender: string;
}) {
    ...
    try {
      const res = await fetch("/api/ai-coach", {
        ...
        body: JSON.stringify({
          ...
          userContext: { position, recentStats: `${tier} drill tier` },
        }),
      });
      if (res.ok) {
        const json = await res.json();
      ...
      <div className="px-4 py-3 min-h-20 max-h-72 overflow-y-auto space-y-3">
        {messages.length === 0 && (
          <p className="text-xs text-gray-400 italic">
            e.g. "How do I improve my first touch?" or "What should I work on this week?"
          </p>
        )}
        {messages.map((m, i) => (
          <div key={i}>
            <div className="text-[9px] font-black uppercase mb-1" style={{ color: m.role === "user" ? "#888" : "#1c3d22" }}>
              {m.role === "user" ? "You" : coachName}
            </div>
