...
import { ArrowLeft, ShieldCheck, Upload, Clock, CheckCircle2, XCircle, Camera, FileText } from "lucide-react";
...
const STATUS_CONFIG: Record<VerifStatus, { icon: typeof ShieldCheck; label: string; color: string; bg: string; desc: string }> = {
  not_submitted: {
    ...
    bg: "bg-muted",
    desc: "Submit a valid ID document to become a verified player. Verification increases trust with scouts.",
  },
  pending: {
    icon: Clock,
    label: "Under review",
  ...
  approved: {
    ...
    bg: "bg-green-500/10",
    desc: "Your identity is verified. Scouts can see your verified badge on your profile.",
  },
  rejected: {
    icon: XCircle,
    label: "Rejected",
    color: "text-red-600",
    bg: "bg-red-500/10",
    desc: "Your submission was rejected. Please re-submit with a clearer document image.",
  },
};
...
export default function PlayerVerificationPage() {
  ...
  useEffect(() => {
    ...
    api.get("/verification/status")
      ...
      .catch(() => setVerif({ status: "not_submitted", document_type: null, ai_confidence_score: null, admin_notes: null, created_at: null, reviewed_at: null }))
          ...
          {(status === "not_submitted" || status === "rejected") && !submitted && (
            <div className="rounded-xl border bg-card p-5">
              <h2 className="mb-4 font-semibold">
                {status === "rejected" ? "Re-submit document" : "Submit your document"}
              </h2>
              ...
              <div className="mb-4 rounded-xl bg-muted/40 px-4 py-3 text-xs text-muted-foreground">
                Your document is processed by AI and reviewed by our admin team. It is never shared with scouts or third parties. We only confirm identity — no data is stored beyond what is needed for verification.
              </div>
