...
import { ArrowLeft, Move, ChevronDown, ChevronUp, RotateCcw, History, Video } from "lucide-react";
...
interface HistoryEntry {
  ...
  cushioning_score:      number;
  touch_direction_score: number;
  speed_of_play_score:   number;
  overall_score:         number;
...
export default function FirstTouchPage() {
  ...
  const fetchCoachRating = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const match = list.find((x: { skill_code: string }) => x.skill_code === "first_touch");
  ...
  const analyseVideoFile = async () => {
    ...
    try {
      const r = await fetch("/api/fitness-test?test_type=first_touch&age_group=u17", {
        method: "POST",
        body: form,
      });
  ...
  const fetchHistory = async () => {
    ...
    setHistLoading(true);
    try {
      const r = await fetch(`${API_URL}/player/first-touch`, { headers: { Authorization: `Bearer ${useAuthStore.getState().token ?? ""}` } });
...
Return this exact JSON structure:
{
  ...
  "pressure_tip": "<one practical tip for receiving under ${pressureLevel.toLowerCase()} pressure with a ${receiveType.toLowerCase()}>"
    ...
    postToArena(`Analysed ${receiveType} first touch under ${pressureLevel} pressure — scored ${overallScore}/100`, {
      postType: "milestone",
      activityType: "skill_analysis",
      activityData: { skill: "first-touch", type: receiveType, score: overallScore, sport },
    });
    setPhase("results");
    setLoading(false);
  };
                ...
                <div key={entry.id} style={{ backgroundColor: "white", borderRadius: 12, border: "1px solid #e5e7eb", marginBottom: 10, overflow: "hidden" }}>
                  <button onClick={() => setOpenHist(isOpen ? null : entry.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ margin: "0 0 2px", fontSize: 14, fontWeight: 600, color: "#111" }}>{entry.sport} · {entry.receive_type} · {entry.pressure_level} pressure</p>
                    ...
                    <div style={{ padding: "0 16px 16px", borderTop: "1px solid #f3f4f6" }}>
                      <div style={{ paddingTop: 14 }}>
                        {[
                          ...
                          { label: "Cushioning", score: entry.cushioning_score, color: "#2563eb" },
                          { label: "Touch Direction", score: entry.touch_direction_score, color: "#c8962a" },
                          { label: "Speed of Play", score: entry.speed_of_play_score, color: "#7c3aed" },
                        ].map((m) => {
                      ...
                      {entry.ai_feedback?.pressure_tip && (
                        <p style={{ margin: "10px 0 0", fontSize: 12, color: "#6b7280" }}><strong>Pressure tip:</strong> {entry.ai_feedback.pressure_tip}</p>
                      )}
                    </div>
                  )}
                </div>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: "28px 16px" }}>
          ...
          {inputMode === "video" && (
            <div style={{ border: "2px dashed #16a34a", borderRadius: 12, padding: "24px 16px", marginBottom: 20, textAlign: "center", backgroundColor: "#f0fdf4" }}>
              ...
              <p style={{ margin: "0 0 12px", fontSize: 13, color: "#374151", fontWeight: 500 }}>Upload a short first touch clip (max 60s)</p>
              ...
              <p style={{ margin: "10px 0 0", fontSize: 12, color: "#6b7280" }}>AI measures body shape &amp; cushioning angle. Rate the rest manually.</p>
            </div>
          )}
