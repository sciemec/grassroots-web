        ...
        {step === 'results' && result && cardData && (
          <div className="space-y-4">
            ...
            {result.suggestedDrills.length > 0 && (
              <div className="bg-white rounded-2xl p-4 border border-gray-100">
                <div className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Drills for this week</div>
                {result.suggestedDrills.map(d => (
                  <div key={d.id} className="flex items-start gap-3 py-2.5 border-b border-gray-50 last:border-b-0">
                    <div className="w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0" style={{ background: '#c8962a' }} />
                    <div>
                      <div className="text-sm font-medium text-gray-800">{d.name}</div>
                      <div className="text-xs text-gray-400">{d.duration} · {d.reason}</div>
                    </div>
                  </div>
                ))}
                <button onClick={() => router.push('/player/drills')}
                  className="w-full mt-3 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600">
