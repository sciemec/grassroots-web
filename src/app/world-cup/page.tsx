...
export default function WorldCupPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { setBallPosition(prev => ({ x: Math.min(95, Math.max(5, prev.x + (Math.random() - 0.5) * 3)), y: Math.min(95, Math.max(5, prev.y + (Math.random() - 0.5) * 2)) })); }, 3000);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      {/* Header */}
      <header className="bg-[#1a5c2a] text-white px-4 py-3 sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-1"><span className="text-xl font-black">Grass<span className="text-[#f0b429]">Roots</span> Sports</span></div>
          <div className="flex items-center gap-4"><div className="flex items-center gap-1 border border-white/20 bg-white/10 rounded px-2 py-0.5 text-xs"><span>🇿🇼</span><ChevronDown size={12} /></div><button className="text-white"><Menu size={24} /></button></div>
        </div>
      </header>

      {/* Promo */}
      <section className="bg-[#f0b429] text-[#1a5c2a] px-4 py-2.5 text-center"><div className="max-w-7xl mx-auto flex items-center justify-center gap-1 text-xs font-black uppercase"><span>🏆</span> FIFA World Cup 2026 - Live AI Commentary & 2D Tracking <span>🏆</span></div></section>

      <main className="max-w-7xl mx-auto px-4 py-6">
        {/* Live count & refresh */}
        <div className="flex justify-between items-center mb-4"><div className="flex items-center gap-2"><Radio size={14} className="text-red-500 animate-pulse" /><span className="text-sm font-bold">{liveCount} live matches</span></div><button onClick={loadData} className="text-xs text-gray-500 flex items-center gap-1"><RefreshCw size={12} /> Refresh</button>{lastUpdated && <span className="text-[9px] text-gray-400">Updated: {lastUpdated.toLocaleTimeString()}</span>}</div>

        {error && <div className="bg-red-50 p-4 rounded-xl text-red-600 text-center mb-4">{error}</div>}
        {isLoading && <div className="flex justify-center py-10"><div className="w-8 h-8 border-2 border-[#1a5c2a] border-t-transparent rounded-full animate-spin" /></div>}

        {!isLoading && matches.length > 0 && (
          <div className="grid lg:grid-cols-3 gap-6">
            {/* LEFT: Match list */}
            <div className="lg:col-span-1 space-y-2">
              <h2 className="text-gray-700 font-bold text-sm uppercase">MATCH SCHEDULE</h2>
              <div className="space-y-2 max-h-[70vh] overflow-y-auto">
                {matches.map(m => (
                  <button key={m.id} onClick={() => setSelectedMatch(m)} className={`w-full text-left p-3 rounded-xl transition-all ${selectedMatch?.id === m.id ? 'bg-[#1a5c2a] text-white border-l-4 border-[#f0b429]' : 'bg-white border border-gray-200 hover:shadow-md'}`}>
                    <div className="flex justify-between"><div><div className="font-bold">{m.homeTeam}</div><div className="text-sm opacity-80">{m.awayTeam}</div></div><div className="text-right">{m.status === 'live' ? <span className="text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full animate-pulse">LIVE {m.minute}</span> : <span className="text-xs text-gray-500">{m.time}</span>}</div></div>
                    {(m.status === 'live' || m.status === 'finished') && <div className="mt-2 text-lg font-black text-[#f0b429]">{m.homeScore} - {m.awayScore}</div>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              {!selectedMatch ? <div className="bg-white rounded-xl p-8 text-center"><Radio size={32} className="mx-auto text-gray-300 mb-3" /><p className="text-gray-500">Select a match to start</p></div> : (
                <>
                  {/* Payment gate */}
                  {!hasAccess && (selectedMatch.status === 'live' || selectedMatch.status === 'finished') ? (
                    <div className="bg-white rounded-xl p-6 text-center border border-gray-200">
                      ...
                      <p className="text-gray-700 mb-2">Unlock live commentary & 2D pitch for this match</p>
                      <button onClick={() => setShowPayment(true)} className="px-6 py-2 bg-[#1a5c2a] text-white rounded-lg font-bold">Unlock for ${selectedMatch.status === 'live' ? '0.99' : '0.49'}</button>
                    </div>
                  ) : (
                    <>
                      {/* Scoreboard */}
                      <div className="bg-white rounded-xl p-5 shadow-md border border-gray-200">
                        <div className="flex justify-between items-center">
                          <div className="text-center flex-1"><span className="text-gray-400 text-xs uppercase">HOME</span><p className="text-gray-800 font-bold text-2xl">{selectedMatch.homeTeam}</p><p className="text-5xl font-black text-[#1a5c2a]">{selectedMatch.homeScore}</p></div>
                          <div className="text-center px-4"><div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center"><span className="text-gray-500 text-xs">VS</span></div><div className="mt-2 text-[11px] text-gray-500">{isLive ? selectedMatch.minute : selectedMatch.time}</div></div>
                          <div className="text-center flex-1"><span className="text-gray-400 text-xs uppercase">AWAY</span><p className="text-gray-800 font-bold text-2xl">{selectedMatch.awayTeam}</p><p className="text-5xl font-black text-[#1a5c2a]">{selectedMatch.awayScore}</p></div>
                        </div>
                        <div className="flex justify-center gap-2 mt-4 text-xs text-gray-500 border-t border-gray-100 pt-3"><MapPin size={12} /> {selectedMatch.stadium}, {selectedMatch.city}</div>
                      </div>

                      {/* 2D Pitch */}
                      <div className="bg-white rounded-xl p-2 shadow-md border border-gray-200">
                        <div className="flex justify-between items-center px-2 pt-2"><span className="text-[10px] text-gray-500 flex items-center gap-1"><Tv size={12} /> TACTICAL VIEW</span><span className="text-[9px] text-gray-400">live tracking</span></div>
        ...
        {/* Footer */}
        <div className="mt-8 text-center text-[9px] text-gray-400 border-t border-gray-200 pt-4">Data: ESPN API • GrassRoots Sports AI • Live Commentary</div>
      </main>

      {/* Floating WhatsApp */}
      <div className="fixed bottom-6 right-4 z-50"><a href="https://wa.me/263775285106" className="w-14 h-14 bg-[#25d366] text-white rounded-full shadow-xl flex flex-col items-center justify-center hover:scale-105 transition"><MessageCircle size={20} /><span className="text-[8px] font-black mt-0.5">WhatsApp</span></a></div>

      {/* Payment Modal */}
      {showPayment && selectedMatch && <PaymentModal matchId={selectedMatch.id} matchName={`${selectedMatch.homeTeam} vs ${selectedMatch.awayTeam}`} onClose={() => setShowPayment(false)} onSuccess={() => { localStorage.setItem(`wc_access_${selectedMatch.id}`, 'true'); setHasAccess(true); setShowPayment(false); }} />}
    </div>
  );
}